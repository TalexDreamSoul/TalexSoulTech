export const SITE_CONTENT = {
  ...
  planning: {
    title: "27 学科与 810 物品：全量运行图",
    lead: "当前生产候选保留 150 个既有运行记录，并为 810 个策划条目建立一对一运行映射；其中 34 项复用经验证的旧身份、776 项新增稳定 ID，最终形成 926 个唯一 runtime。九个波次、27 个学科、270 个家族均可独立验收。",
    accounting: {
      implementedRoots: 27,
      plannedRoots: 0,
      plannedItemCapacity: 810,
      clarification: "810 个策划 ID 已全部实装；实际命令、PDC 与发布身份仍以 /runtime 的 926 个唯一记录为准。",
    },
    principles: [
      "先补齐一条可循环的生产链，再新增相邻学科；不以孤立收藏品填充目录。",
      "每个新学科都必须说明前置、产出、消耗、回收和失败成本，避免单向通胀。",
      "跨学科配方只能制造新的决策，不应把所有旧线压缩成唯一最优解。",
      "策划 ID 与运行 ID 分栏显示；所有 810 项均有可验证配方、行为或设施边界。",
...
export const TUTORIALS = [
  ...
  },
  {
    ...
    steps: [
      ...
      { title: "按波次学习", detail: "从 W1 到 W9 进入 27 个学科与 270 个家族；同波次学科并行，只有波次顺序是硬门禁。" },
      { title: "保留阅读上下文", detail: "重新使用向导书时，系统优先恢复 lastGuider；没有记录时进入完整向导目录。" },
      { title: "识别运行身份", detail: "向导展示策划 ID、配方与行为；实际命令和 PDC 身份以对应 runtime ID 为准，二者不得混用。" }
    ],
